...
export default function Leads() {
  ...
  useEffect(() => {
    get('/api/leads')
      .then(data => setLeads(Array.isArray(data) ? data : data?.items || []))
      .catch(() => {})
  ...
  const handleCreate = async () => {
    setCreating(true)
    try {
      const created = await post('/api/leads', form)
    ...
    <div className="space-y-6 animate-slide-up">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/50 text-sm mt-1">Manage your prospect database</p>
        </div>
        <button
      ...
      {leads.length === 0 ? (
        <EmptyState
          icon={Users}
          title="No leads yet"
          message="Add leads to start your outbound campaigns."
          action="Add Lead"
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Add Lead">
