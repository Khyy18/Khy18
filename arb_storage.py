...
def get_active() -> Optional[dict[str, Any]]:
    """Вернуть единственную активную позицию (OPEN или CLOSING). None если нет.

    Принципиально: executor поддерживает не больше одной арб-пары
    одновременно. Если в БД случайно оказалось >1 - возвращаем самую
    свежую и логируем (это означает кривое состояние).
    """
    try:
        with _connect() as conn:
            rows = conn.execute(
        ...
        if len(rows) > 1:
            print(
                f"[ARB-STORE] WARNING: найдено {len(rows)} активных пар; "
                "возвращаю самую свежую. Очистите БД руками."
            )
        return dict(rows[0])
    except sqlite3.Error as exc:
        print(f"[ARB-STORE] get_active fail: {exc}")
        return None
