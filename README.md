...
Система реализует многоуровневую защиту данных:

1. **Шифрование данных (Fernet: AES-128-CBC + HMAC-SHA256)** - персональные данные сотрудников и детей шифруются перед сохранением в базу. Ключ шифрования задается через переменную окружения `ENCRYPTION_KEY`.

2. **Двухфакторная аутентификация и биометрия** - мобильное приложение поддерживает PIN-код и биометрическую аутентификацию (отпечаток пальца, Face ID) через secure storage. Экран блокировки активируется при запуске.

3. **Журнал аудита** - все операции создания, изменения и удаления данных автоматически фиксируются в таблице `audit_log` с указанием времени, IP-адреса, типа действия и измененной сущности. Доступ к журналу через `GET /api/v1/audit` (только для ролей admin и director).

4. **Ограничение частоты запросов (Rate Limiting)** - защита от DDoS и брутфорса через slowapi с лимитом 60 запросов в минуту на IP-адрес.

5. **Ролевой доступ (RBAC)** - три роли: admin, cashier, director. Разграничение через заголовок `X-User-Role`. Критические операции (аудит, удаление) доступны только администраторам.

6. **Мониторинг Sentry** - интеграция с Sentry для отслеживания ошибок и производительности в реальном времени. Настраивается через `SENTRY_DSN`.

7. **Одноразовые ссылки на скачивание** - экспортированные файлы (Excel-ведомости, отчеты) доступны по одноразовому токену с автоматическим истечением через 10 минут. После первого скачивания ссылка удаляется.

8. **Автоудаление конфиденциальных файлов** - Telegram-бот автоматически удаляет сообщения с финансовыми данными через заданный интервал для предотвращения утечки через историю чата.
