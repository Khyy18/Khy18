...
## v3 Telegram UI (дашборд + Демо/Реал + замена ключей)

Ветка `feat/v3-tg-dashboard` расширяет v3 большим Telegram-дашбордом и
возможностью управлять ботом целиком из чата, без SSH.

### Главное меню: 9 кнопок (4 ряда по 2 + 1 широкая)
...
- 📊 СТАТУС - сводный дашборд: баланс, винрейт, режимы по 7 символам,
  AI-Gate статистика за 24ч, DRY_RUN, режим торговли, kill-switch, uptime.
  Inline-кнопки: «Переключить DRY_RUN», «Режим AI-Gate», «Демо/Реал»,
  «Снять kill-switch» (если активен).
- 📈 ПОЗИЦИИ - открытые позиции с подтянутыми текущими ценами, PnL,
  временем удержания, стопом. Сортировка по PnL по убыванию.
- 🛡 AI-GATE - текущий режим (активен / наблюдение / выключен), статистика
  за 24ч и 7 дней, топ причин блокировки. Inline: переключение режима
  с two-step confirm, просмотр последних 10 решений.
- 🎯 РЕЖИМЫ - классификация рынка (Тренд / Боковик / Кризис) по 7 символам
  с уверенностью и причиной от ai_regime.
- 🔍 ПОЧЕМУ МИМО? - группировка отклонённых сигналов за 24ч (с переводом
  технических тегов на русский) + последнее отклонение. Inline: запрос
  AI-разбора, фильтр «только AI-Gate блокировки».
- 📝 ЛОГИ - последние 40 строк `journalctl -u zenith`, с inline-фильтрами
  «Только ошибки» и «Только AI-Gate».
- ⏯ СТАРТ/СТОП - подменю: старт/пауза торговли, переключение DRY_RUN,
  PANIC SELL.
- 🚨 PANIC SELL - two-step confirm, затем reduce-only маркет по всем
  открытым позициям + пауза торговли.
- 🔑 КЛЮЧИ API - безопасная замена любого из 9 ключей прямо из чата.

### Переключатель Демо/Реал (кнопка 🏦 Демо/Реал в 📊 СТАТУС)

Бот поддерживает две раздельные тройки OKX-ключей в `.env`:

- `OKX_API_KEY` / `OKX_API_SECRET` / `OKX_PASSPHRASE` - демо-счёт OKX
  (виртуальный баланс, `x-simulated-trading: 1`).
- `OKX_API_KEY_REAL` / `OKX_API_SECRET_REAL` / `OKX_PASSPHRASE_REAL` -
  боевой счёт OKX mainnet.

Активная тройка выбирается по значению `IS_TESTNET` в `.env`. Переключение
через кнопку записывает новое значение в `.env` и перезапускает процесс
(SIGTERM, systemd-unit с `Restart=always` поднимает бот через ~20 секунд).

**Рекомендованный порядок перехода на реал:**

1. Заполните `OKX_API_KEY_REAL` / `OKX_API_SECRET_REAL` / `OKX_PASSPHRASE_REAL`
   через 🔑 КЛЮЧИ API → OKX Real *.
2. Поставьте `DRY_RUN=true` (через ⏯ СТАРТ/СТОП → «Переключить DRY_RUN»).
3. Переключитесь на РЕАЛ через 📊 СТАТУС → 🏦 Демо/Реал. Бот перезапустится.
4. Убедитесь что 📊 СТАТУС показывает реальный баланс вашего mainnet-счёта.
5. Только после этого переключите `DRY_RUN=false` - бот начнёт
   отправлять реальные ордера.

### Кнопка 🔑 КЛЮЧИ API - замена всех 9 ключей

Замена поддерживается для:

- **OKX Демо** (активны при `IS_TESTNET=true`): Key / Secret / Passphrase.
- **OKX Реал** (активны при `IS_TESTNET=false`): Key / Secret / Passphrase.
- **Общие**: Groq API Key, NewsAPI Key, Telegram Token.

Особенности безопасности:

- Значение ключа пересылается одним сообщением, которое бот немедленно
  удаляет через `deleteMessage` (ключ не остаётся в истории чата).
- Значение никогда не логируется в stdout и не пишется в БД - только в
  `.env` через атомарную запись (tempfile + `os.replace`, права `0o600`).
- Предусмотрена минимальная проверка длины (OKX-ключи ≥ 32 символа,
  passphrase ≥ 6, Groq/NewsAPI ≥ 20, TELEGRAM_TOKEN ≥ 40 и содержит `:`).
- Two-step confirm перед записью; таймаут FSM 5 минут; per-chat_id state.
- После подтверждения бот посылает себе SIGTERM - systemd перезапустит
  процесс, он перечитает `.env` и поднимется с новыми ключами.

⚠ **Смена TELEGRAM_TOKEN** приведёт к потере связи с ботом в текущем чате:
у `@BotFather` под новый токен создаётся новый бот. Восстановление -
только через SSH на сервер: `sudo -u zenith nano /opt/zenith/.env`.
