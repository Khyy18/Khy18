...
def _select_candidate(
    ...
    Фильтры:
      ...
      - символ НЕ в excluded_symbols (уже занятые открытыми позициями);
...
async def monitor_and_maybe_close(
    ...
    Условия закрытия (применяются к каждой паре независимо):
      ...
      3. config.ARB_EXECUTOR_ENABLED стал False.
    """
    ...
    for pos in positions:
        ...
        # Условие 3: executor выключили из конфига.
        if not getattr(config, "ARB_EXECUTOR_ENABLED", False):
            closed = await _force_close(
...
async def reconcile_funding_payments(
    ...
    for pos in positions:
        ...
        for snap, sign_for_us in [(long_snap, +1.0), (short_snap, -1.0)]:
            # funding конвенция: положительный rate = LONG ПЛАТИТ shorts.
            #   LONG-нога: funding нам = -rate * notional
            #   SHORT-нога: funding нам = +rate * notional
