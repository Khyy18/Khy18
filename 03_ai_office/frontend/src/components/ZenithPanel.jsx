...
/**
 * ZenithPanel - панель интеграции с Zenith Crypto Trading
 * Показывает статус сервиса, торговые метрики
 */
export default function ZenithPanel() {
  const { data, loading, error } = useApi('services/zenith', 10000)
      ...
      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-white">Zenith Crypto Trading</h3>
          </div>
        ...
        {!isOnline && !loading && (
          <p className="text-white/40 text-xs">
            Service is not reachable. Make sure Zenith Crypto Trading is running.
          </p>
        )}
      </div>
      ...
      {isOnline && stats && (
        <div className="grid grid-cols-2 gap-3">
          ...
          <MetricCard
            icon={DollarSign}
            label="Daily PnL"
            value={stats.daily_pnl != null ? `${stats.daily_pnl} USDT` : '-'}
