...
/**
 * TextAgencyPanel - панель интеграции с AI Text Agency
 * Показывает статус сервиса, метрики контента
 */
export default function TextAgencyPanel() {
  const { data, loading, error } = useApi('services/text-agency', 10000)
      ...
      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-white">AI Text Agency</h3>
          </div>
        ...
        {!isOnline && !loading && (
          <p className="text-white/40 text-xs">
            Service is not reachable. Make sure AI Text Agency is running.
          </p>
        )}
      </div>
      ...
      {isOnline && stats && (
        <div className="grid grid-cols-2 gap-3">
          ...
          <MetricCard
            icon={TrendingUp}
            label="Engagement"
            value={stats.avg_engagement != null ? `${stats.avg_engagement}%` : '-'}
