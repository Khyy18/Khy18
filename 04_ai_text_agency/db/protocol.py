...
class StorageProtocol(Protocol):
    """Интерфейс хранилища торгового бота (синхронный).

    Все методы синхронные - реализуется SQLiteBackend.
    """
...
class AsyncStorageProtocol(Protocol):
    """Асинхронный интерфейс хранилища (для PostgresBackend).

    Все методы async def - для использования в async-контексте.
    PostgresBackend реализует этот протокол. Вызывающий код
    должен использовать await при работе с этим бэкендом.
    """
